...
export default function StaysDetail() {
  ...
  const canBook = !!trip.checkIn && !!trip.checkOut && nights > 0 && trip.guests > 0;

  function handleBooking() {
    if (!canBook) return;

    const params = new URLSearchParams({
      checkIn: trip.checkIn!.toISOString(),
      checkOut: trip.checkOut!.toISOString(),
      guests: trip.guests.toString(),
    });

    navigate(`/booking/stay-1?${params.toString()}`);
  }
